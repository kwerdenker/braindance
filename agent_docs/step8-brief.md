...
falling through to the static file server — which is what stops `/library/../web/main.js`
being served as a file. `jobs` is not in that regex, so `/jobs/...` falls through and
`/jobs/../web/main.js` is a file lookup rather than an API 404. Adding `jobs` to the
literal would fix the instance and leave the next namespace anybody adds outside the list,
which is the failure mode `CLAUDE.md` names by name. **Derive the list from `ROUTES`**
...
The falsification control this needs is the one that shape always needs: a **planted route
under a namespace not in the literal list**, which must FAIL. A control that merely moves
an existing route cannot see it, for the same reason `stop-route-reads` could not see a
planted read route that writes — a count of registered routes does not answer "was this
path reachable as a file".
